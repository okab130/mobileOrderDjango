...
- 機能:
  - 注文一覧表示（3つの表示モード切り替え）
    - **未提供**: 提供済以外の全注文（調理管理用）
      - 表示項目: テーブル番号、商品名、数量、注文時刻、経過時間、ステータス
      - ステータス変更ボタン付き
    - **未会計・未提供**: 未会計かつ未提供の注文
      - 表示項目: テーブル番号、商品名、数量、注文時刻、単価、小計、ステータス
      - 閲覧のみ（ステータス変更不可）
  ...
  - 呼び出し対応済みボタン
  - 会計・レシート印刷
    - 未会計セッション一覧表示（セッションID、テーブル、人数、合計金額）
    - セッション選択機能
    - 会計・レシート印刷ボタン（自動的に会計完了処理を実行）
...
### 3.2 店舗管理画面（PC）
1. ログイン画面（`/management/login/`）
2. ダッシュボード（`/management/dashboard/`）
   - 注文一覧（未提供/未会計・未提供/未会計・全注文）
   - 店員呼び出し一覧
   - 会計・レシート印刷
3. メニュー管理画面（`/management/menu/`）- 管理者のみ
4. 売上管理画面（`/management/sales/`）- 管理者のみ
5. ユーザー管理画面（`/management/users/`）- 管理者のみ
